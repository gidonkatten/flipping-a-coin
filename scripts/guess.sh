...
HEADS=1
# create transactions
${gcmd} app call --app-id "$APP_ID" --app-arg "str:guess" --app-arg "int:$HEADS" -f "$GUESSER" -o unsignedtx0.tx
${gcmd} clerk send -a "$AMOUNT" -f "$GUESSER" -t "$ESCROW_ADDRESS" -o unsignedtx1.tx
...
# read global state
${gcmd} app read --app-id "$APP_ID" --guess-format --global --from "$GUESSER"
